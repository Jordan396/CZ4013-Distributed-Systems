...
bool CacheService::monitorFile(std::string pathName, int monitorDuration) {
  ...
  int res = client.register_client(pathName, getLocalPathToFile(pathName),
                                   to_string(monitorDuration));
  ...
  if (it == cacheMap.end()) {
    ...
    if (file.computeTimeElapsed() <= freshnessInterval * 1.0 / (1000 * 1.0)) {
      ...
      if (difftime(last_modified_time, file.createdTime) != 0) {
        Debug::msg("Server and client last modified time does not match, "
                   "server has more updated copy");
        return fetchFile(pathName);
      } else {
